...
class TCPServer:
    def __init__(self, pFolder=None, index=0):
        ...
        self.logFile = open(
            os.environ.get("GUI_dir") + "/Gui/.tmp/" + str(self.index) + ".txt", "w"
        )
    ...
    def start(self, pConfigFile):
        ...
        if self.folder is not None:
            print(
                "Ph2_ACF_GUI:\tTCP Server: Start TCP Device Server at "
                + self.folder
                + " with config file "
                + self.config
            )
            if not self.isRunning():
                ...
                cmd = (
                    ...
                    + "/power_supply/bin/PowerSupplyController -c "
                    + self.config
                    + " -p "
                    + str(7000 + self.index)
                )
    ...
    def checkDeviceSubProcess(self):
        ...
        if self.serverProcess is not None:
            ...
            print(
                "Ph2_ACF_GUI:\tCheck subprocess of device "
                + self.config
                + ": is running"
            )
    ...
    def stop(self):
        print(
            "TCP Server: Kill TCP Device Server at "
            + self.folder
            + " with config file "
            + self.config
        )
...
class TCPClient(QObject):
    ...
    def __init__(self, index=0):
        ...
        self.lib = ctypes.cdll.LoadLibrary(
            os.environ.get("GUI_dir") + "/power_supply/NetworkUtils/NetworkUtils.so"
        )
    ...
    def decodeStatus(self, pStatus):
        ...
        timestamp = (
            splitTimestamp[1] + ":" + splitTimestamp[2] + ":" + splitTimestamp[3]
        )
