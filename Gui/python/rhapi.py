...
class CernSSO:
    ...
    def read_form(self, r):
        soup = self.html_root(r)
        form = soup.find("form")
        if not form:
            raise ValueError("No form found in the response. Retry then contact developer")
...
class RhApi:
    ...
    def data1(self, query, params=None, form='text/csv', pagesize=None, page=None, verbose=False, cols=False,
          inline_clobs=False):

        for attempt in range(5):
            try:
                qid = self.qid(query)
                a = self.data(qid, params, form, pagesize, page, verbose=verbose, cols=cols, inline_clobs=inline_clobs)
                return a
    ...
    def csv(self, query, params=None, pagesize=None, page=None, verbose=False, inline_clobs=False):
        ...
        """
        # qid = self.qid(query)
        return self.data1(query, params, 'text/csv', pagesize, page, verbose=verbose, inline_clobs=inline_clobs)
    ...
    def xml(self, query, params=None, pagesize=None, page=None, verbose=False, inline_clobs=False):
        ...
        """
        # qid = self.qid(query)
        return self.data1(query, params, 'text/xml', pagesize, page, verbose=verbose, inline_clobs=inline_clobs)
    ...
    def json(self, query, params=None, pagesize=None, page=None, verbose=False, cols=False, inline_clobs=False):
        ...
        """
        # qid = self.qid(query)
        return self.data1(query, params, 'application/json', pagesize, page, verbose=verbose, cols=cols,
