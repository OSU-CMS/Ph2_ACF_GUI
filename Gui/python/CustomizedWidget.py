...
class BeBoardBox(QWidget):
    ...
    def getFirmwareDescription(self, **kwargs):
        print("In getFirmware Description")
        for index, module in enumerate(self.ModuleList):
            FwModule = QtModule()
...
class SimpleBeBoardBox(QWidget):
    ...
    def getFirmwareDescription(self, **kwargs):
        print("FILLEDMODULELIST: ", self.FilledModuleList)
        for index, module in enumerate(self.FilledModuleList):
            if module.getSerialNumber() == "":
                continue
            print(module.getSerialNumber())
            FwModule = QtModule()
            FwModule.setModuleID(module.getID())
            FwModule.setFMCID(module.getFMCID())
            FwModule.setModuleName(module.getSerialNumber())
            ## TODO getType() expects an ID associated with it if id starts with zh ...
            for chip in ModuleLaneMap[
                module.getType(module.getSerialNumber())
            ].values():
                FwModule.setChipStatus(chip, 1)
                # Change this! The VDDA and VDDD should be obtained from database
                FwModule.setChipVDDA(chip, 6)
                FwModule.setChipVDDD(chip, 6)
