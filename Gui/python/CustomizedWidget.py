        ...
        for module in self.ModuleList:
            ...
            if chipData:
                for chipID in ModuleLaneMap[
                    ...
                    iref = chipData[chipID].get("IREF", "")
                    vref = chipData[chipID].get("VREF", "800")
                    if iref not in (None):
                        Module.getChips()[chipID].setIREF(int(iref))
                    if vref not in (None):
                        Module.getChips()[chipID].setVREF(int(round(1000 * float(vref))))
