...
class QtViewTableTab(QWidget):
    ...
    def searchBox(self):
        ...
        for i in range(self.TableCombo.count()):
            if (
                self.TableCombo.itemText(i) == "images"
                or self.TableCombo.itemText(i) == "tests"
            ):
    ...
    def showSearchResult(self):
        ...
        try:
            dataList = [
                describeTable(self.connection, self.TableCombo.currentText(), True)
            ]
            if self.ConditionEdit.text() == "":
                dataList += retrieveGenericTable(
                    self.connection, self.TableCombo.currentText()
                )
            else:
                dataList += retrieveWithConstraintSyntax(
                    self.connection,
                    self.TableCombo.currentText(),
