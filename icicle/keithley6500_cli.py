...
@click.option(
    ...
    type=str,
    default="ASRL6::INSTR",
    help="VISA resource address (default: ASRL6::INSTR)",
)
@click.option(
    "-S",
    "--simulate",
...
@click.pass_context
def cli(ctx, resource, verbose, simulate, cls=Keithley6500):
...
@cli.command(
    "measure",
    help=(
        "Measure voltage (VOLT:DC / VOLT:AC), current (CURR:DC / CURR:AC), resistance "
        "(RES), four-wire resistance (FRES), frequency (FREQ), or period (PER). Also "
...
@cli.command(
    "set",
    help=(
        "Set configuration field FIELD to value VALUE (see keithley6500.py for "
        "config table)."
    ),
)
...
@cli.command(
    "query",
    help="Query configuration field FIELD (see keithley6500.py for config table).",
)
@click.argument("field", metavar="FIELD", type=str)
@with_instrument
@print_output
