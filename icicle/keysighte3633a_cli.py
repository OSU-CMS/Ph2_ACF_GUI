...
@click.option(
    ...
    type=str,
    default="ASRL4::INSTR",
    help="VISA resource address (default: ASRL4::INSTR)",
)
@click.option(
    "-S",
    "--simulate",
...
@click.pass_context
def cli(ctx, resource, verbose, outputs=1, simulate=False, cls=KeysightE3633A):
    logging.basicConfig(level=verbosity(verbose))
    ctx.obj = cls(resource=resource, outputs=outputs, sim=simulate)


@cli.command("on", help="Turn output ON for channel CHANNEL")
@click.argument("channel", metavar="CHANNEL", type=int, default="1")
@with_instrument
def cli_on(instrument, channel):
    return instrument.on(int(channel))


@cli.command("off", help="Turn output OFF")
@click.argument("channel", metavar="CHANNEL", type=int)
@with_instrument
def cli_off(instrument, channel):
    return instrument.off(int(channel))
...
@cli.command(
    "voltage",
    help=(
        "Set VOLTAGE (V) for channel CHANNEL, or query current value if VOLTAGE not "
        "provided."
    ),
)
@click.argument("channel", metavar="CHANNEL", type=int, default=1)
...
@cli.command(
    "current",
    help=(
        "Set CURRENT (A) for channel CHANNEL, or query current value if CURRENT not "
        "provided."
    ),
)
@click.argument("channel", metavar="CHANNEL", type=int, default=1)
...
@click.argument(
    "channel", metavar="CHANNEL", type=click.Choice(("1", "2", "3", "ALL")), default=1
)
@click.option(
    "-d",
    "--delay",
...
@with_instrument
def cli_monitor(
    instrument, channel, delay, output, mqtt, clientid, broker, port, username, password
):
    SPACING = 17
    ...
    if mqtt:
        ...
        client.loop_start()
        log.register_writer(
            lambda line, end="": instrument.publish(client, line, channel)
        )
...
@cli.command(
    "set",
    help=(
        "Set configuration field FIELD to value VALUE (see keysightE3633A.py for "
        "config table)."
    ),
)
...
@cli.command(
    "query",
    help="Query configuration field FIELD (see keysightE3633A.py for config table).",
)
@click.argument("field", metavar="FIELD", type=str)
@with_instrument
@print_output
