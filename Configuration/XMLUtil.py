...
class BeBoardModule:
    def __init__(self):
        ...
        self.address_table = (
            "file://${PH2ACF_BASE_DIR}/settings/address_tables/CMSIT_address_table.xml"
        )
        self.FEModuleList = []
...
def SetMonitoring(Node, MonitoringItem):
    ...
    Node_Monitoring = SetNodeAttribute(
        Node_Monitoring, {"type": MonitoringItem.Type, "enable": MonitoringItem.Enable}
    )
    ...
    for element in MonitoringItem.MonitoringList.keys():
        ...
        Node_MonitoringElements = SetNodeAttribute(
            Node_MonitoringElements,
            {
                "device": "RD53",
                "register": element,
                "enable": MonitoringItem.MonitoringList[element],
            },
        )
    return Node


def GenerateHWDescriptionXML(
    HWDescription, outputFile="CMSIT_gen.xml", boardtype="RD53A", isOpticalLink=False
):
    ...
    for BeBoard in BeBoardList:
        ...
        Node_BeBoard = SetNodeAttribute(
            Node_BeBoard,
            {
                "Id": BeBoard.Id,
                "boardType": BeBoard.boardType,
                "eventType": BeBoard.eventType,
            },
        )
        ...
        Node_connection = SetNodeAttribute(
            Node_connection,
            {
                "id": BeBoard.id,
                "uri": BeBoard.uri,
                "address_table": BeBoard.address_table,
            },
        )
        ...
        for OGModule in OpticalGroupList:
            ...
            Node_OGModule = SetNodeAttribute(
                Node_OGModule, {"Id": OGModule.Id, "FMCId": OGModule.FMCId}
            )
            if isOpticalLink:
                ...
                Node_lqGBT = SetNodeAttribute(
                    Node_lqGBT,
                    {
                        "Id": "0",
                        "version": "1",
                        "configfile": "CMSIT_LqGBT-v1.txt",
                        "ChipAddress": "0x70",
                        "RxDataRate": "1280",
                        "RxHSLPolarity": "0",
                        "TxDataRate": "160",
            ...
            for HyBridModule in HyBridList:
                ...
                Node_HyBrid = SetNodeAttribute(
                    Node_HyBrid,
                    {
                        "Id": HyBridModule.Id,
                        StatusStr: HyBridModule.Status,
                        "Name": HyBridModule.Name,
                    },
                )
                #### This is where the RD53_Files is setup ####
                ##FIXME Add in logic to change depending on version of Ph2_ACF -> Done!

                HyBridModule.SetHyBridType(
                    "RD53"
                )  # This part should stay as just RD53 (no A or B)
                print("This is the Hybrid Type: ", HyBridModule.HyBridType)
                Node_FEPath = ET.SubElement(
                    Node_HyBrid, HyBridModule.HyBridType + "_Files"
                )
                Node_FEPath = SetNodeAttribute(
                    Node_FEPath, {"file": HyBridModule.File_Path}
                )
                ...
                for FE in FEList:
                    ...
                    Node_FE = SetNodeAttribute(
                        Node_FE,
                        {"Id": FE.Id, "Lane": FE.Lane, "configfile": FE.configfile},
                    )
                    if "RD53B" in boardtype:
                        Node_FELaneConfig = ET.SubElement(Node_FE, "LaneConfig")
                        Node_FELaneConfig = SetNodeAttribute(
                            Node_FELaneConfig,
                            {
                                "primary": "1",
                                "outputLanes": "0001",
                ...
                Node_FEGlobal = SetNodeAttribute(
                    Node_FEGlobal, HyBridModule.globalSetting
                )
...
def GeneratePowerSupplyXML(device, outputFile="PowerSupplyConfig.xml"):
    ...
    for channel in ChannelList:
        ...
        Node_Channel = SetNodeAttribute(
            Node_Channel,
            {
                "ID": channel.getID(),
                "Channel": channel.getChannel(),
                "InUse": channel.getInUse(),
            },
        )
