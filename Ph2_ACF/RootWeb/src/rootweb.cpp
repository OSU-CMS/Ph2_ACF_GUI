...
ostream& RootWTable::dump(ostream& output)
{
    ...
    for(tableContentIt = tableContent_.begin(); tableContentIt != tableContent_.end(); ++tableContentIt)
    {
...
void RootWTable::setColor(int row, int column, int newColor) { tableContentColor_[make_pair(row, column)] = newColor; }
...
void RootWImage::setCanvas(TCanvas& myCanvas) { setCanvas(&myCanvas); }
...
void RootWImage::setRelativeHtmlDirectory(string newDirectory) { relativeHtmlDirectory_ = newDirectory; }

void RootWImage::setTargetDirectory(string newDirectory) { targetDirectory_ = newDirectory; }
...
string RootWImage::saveFiles(int smallWidth, int smallHeight)
{
    ...
    if(fileSaved_[myImageSize]) { return myText_[myImageSize]; }
    ...
    thisText << "<img class='wleftzoom' width='" << imgW << "' height='" << imgH << "' src='" << relativeHtmlDirectory_ << "/" << smallCanvasFileName << "' alt='" << comment_
             << "' onclick=\"popupDiv('" << relativeHtmlDirectory_ << "/" << largeCanvasFileBaseName << "', " << zoomedWidth_ << ", " << zoomedHeight_ << " , '" << comment_ << "','" << fileTypeList
             << "');\" />";
...
bool RootWImage::addExtension(string myExtension)
{
    ...
    else { cerr << "Extension '" << myExtension << "' is not allowed for canvas saving: new file type not added." << endl; }
...
void RootWImage::saveSummaryLoop(TPad* basePad, std::string baseName, TFile* myTargetFile)
{
    ...
    for(int i = 0; i < aList->GetEntries(); ++i)
    {
        ...
        else if((myClass == "TProfile") || (myClass == "TGraph") || (myClass == "TH1D") || (myClass == "TH2C") || (myClass == "TH2D") || (myClass == "THStack"))
        {
        ...
        else if((myClass == "TEllipse") || (myClass == "TFrame") || (myClass == "TLatex") || (myClass == "TLegend") || (myClass == "TLine") || (myClass == "TPaveText") || (myClass == "TPolyLine") ||
                (myClass == "TText"))
        {
        }
        else { std::cerr << Form("Unhandled class %s", myClass.c_str()) << std::endl; }
    }
}
...
void RootWContent::setTargetDirectory(string newTargetDirectory) { targetDirectory_ = newTargetDirectory; }
...
{
    if(newItem) { itemList_.push_back(newItem); }
}
...
ostream& RootWContent::dump(ostream& output)
{
    ...
    if(visible_) { output << "<div class=\"hideable\"> "; }
    else { output << "<div class=\"hidden\"> "; }
    for(it = itemList_.begin(); it != itemList_.end(); ++it)
    {
        ...
        if(myItem->isImage())
        {
            ...
            else { cout << "WARNING: this should never happen. contact the author immediately!" << endl; }
        }
        if(myItem->isFile())
        {
            if((myFile = dynamic_cast<RootWFile*>(myItem)))
            { // CUIDADO This is not good OOP for God's sake!!!
                myFile->setTargetDirectory(targetDirectory_);
            }
            else if(RootWFileList* myFileList = dynamic_cast<RootWFileList*>(myItem)) { myFileList->setTargetDirectory(targetDirectory_); }
            else { cout << "WARNING: this should never happen. contact the author immediately!" << endl; }
        }
        myItem->dump(output);
    }
...
{
    if(page_ == nullptr) { return nullptr; }
    else { return page_->getSummaryFile(); }
}
...
void RootWPage::setTargetDirectory(string newTargetDirectory) { targetDirectory_ = newTargetDirectory; }
...
{
    if(newContent) { contentList_.push_back(newContent); }
}
...
{
    if(site_) { return site_->getSummaryFile(); }
    else { return nullptr; }
}
...
void RootWSite::setCommentLink(string newCommentLink) { commentLink_ = newCommentLink; }
...
void RootWSite::addPage(RootWPage* newPage, int relevance /* = least_relevant */)
{
    ...
    if(relevance == least_relevant) { pageList_.push_back(newPage); }
    else
    {
...
void RootWSite::addAuthor(string newAuthor) { authorList_.push_back(newAuthor); }

ostream& RootWSite::dumpFooter(ostream& output)
{
    output << "          </div>" << endl << "        </div>" << endl << "        <div class=\"clear\"></div>" << endl << "      </div>" << endl << "      <div id=\"footer\">" << endl;

    // Add the list of authors if any
    bool firstAuthorFound = false;
    for(vector<string>::iterator it = authorList_.begin(); it != authorList_.end(); ++it)
    {
        if(!firstAuthorFound)
        {
    ...
    time_t rawtime;
    time(&rawtime);
    output << "        <p>Page created on " << asctime(gmtime(&rawtime)) << " GMT</p>" << endl << "        <p>by <a href=\"" << programSite_ << "\">" << programName_ << "</a>";
    if(revision_ != "") output << " revision " << revision_;
    output << "</p>" << endl << "      </div>" << endl << "    </div>" << endl << "  </body>" << endl << "</html>" << endl;
    return output;
}
...
ostream& RootWSite::dumpHeader(ostream& output, RootWPage* thisPage)
{
    output << "<html xmlns=\"http://www.w3.org/1999/xhtml\">" << endl
           ...
           << "        </h1>" << endl;
    if(commentLink_ == "") { output << "    <h2>" << comment_ << "</h2>" << endl; }
    else
    {
        output << "    <h2>"
    ...
    output << "      </div>" << endl << "      <div id=\"menu\">" << endl << "        <ul>" << endl;
    for(vector<RootWPage*>::iterator it = pageList_.begin(); it != pageList_.end(); ++it)
    {
        output << "          <li";
        ...
        if((*it)->getAlert() > 0)
        {
            ...
            anAlarmColor.str("");
            anAlarmColor << "rgb(255, " << int(255 * (*it)->getAlert()) << ", 0)";
...
bool RootWSite::makeSite(bool verbose)
{
    ...
    if(createSummaryFile_) { summaryFile_ = new TFile(Form("%s/%s", targetDirectory_.c_str(), summaryFileName_.c_str()), "RECREATE"); }
    else
        summaryFile_ = nullptr;
...
TFile* RootWSite::getSummaryFile() { return summaryFile_; }

void RootWSite::setSummaryFile(bool doSummary) { createSummaryFile_ = doSummary; }
...
void RootWSite::setProgram(const std::string& programName, const std::string& programSite)
{
    programName_ = programName;
    programSite_ = programSite;
}
...
void RootWItemCollection::addItem(RootWItem* anItem, string itemName)
{
    ...
    if(findItem != NULL) { cerr << "Warning in RootWItemCollection::addItem(): item named " << itemName << " already exists! Overwriting it." << endl; }
...
ostream& RootWTextFile::dump(ostream& output)
{
    ...
    output << "<b>" << description_ << ":</b> <a href=\"" << fileName_ << "\">" << fileName_ << "</a></tt><br/>";
    return output;
}
...
ostream& RootWBinaryFile::dump(ostream& output)
{
    ...
    if(boost::filesystem::exists(originalFileName_) && originalFileName_ != destinationFileName)
    { // CUIDADO: naive control on copy on itself. it only matches the strings, not taking into account relative paths and symlinks
        try
        {
...
ostream& RootWBinaryFileList::dump(ostream& output)
{
    ...
    for(auto fn: fileNames_)
    {
        ...
        if(boost::filesystem::exists(*it) && *it != destinationFileName)
        { // CUIDADO: naive control on copy on itself.
            try
            {
                if(boost::filesystem::exists(destinationFileName)) boost::filesystem::remove(destinationFileName);
    ...
    std::transform(originalFileNames_.begin(),
                   originalFileNames_.end(),
