...
std::string pxarLog<T>::NowTime()
{
    ...
    char      buffer[MAX_LEN];
    if(GetTimeFormatA(LOCALE_USER_DEFAULT, 0, 0, "HH':'mm':'ss", buffer, MAX_LEN) == 0) return "Error in NowTime()";
...
{
    if(level == "INTERFACE") return logINTERFACE;
    if(level == "DEBUGPIPES") return logDEBUGPIPES;
    if(level == "DEBUGRPC") return logDEBUGRPC;
    if(level == "DEBUGHAL") return logDEBUGHAL;
    if(level == "DEBUGAPI") return logDEBUGAPI;
    ...
    if(level == "QUIET") return logQUIET;
...
inline void SetLogOutput::Output(const std::string& msg)
{
    ...
    if(Duplicate() && pStream != stderr) fprintf(stderr, "%s", msg.c_str());
...
#define LOG(level)                                                                                                                                                                                     \
    if(level > pxar::Log::ReportingLevel() || !pxar::SetLogOutput::Stream())                                                                                                                           \
        ...
        pxar::Log().Get(level, __FILE_NAME__, __func__, __LINE__)
