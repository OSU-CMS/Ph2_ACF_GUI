...
/* For more info and how to use this libray, visit:
...
class RS232Conn
{
    ...
    std::string readSuffix;         // Suffix stripped from read data, also signals end of
                                    // read, to ignore, set to ""
  ...
  public:
    ...
    void setUSBTMCMode(bool mode);

    void writeData(const std::string& data);
    bool readEcho(const std::string& data);
    // returns true if this read reached the end of an input line(ie did not match
    // on readSuffix)
    bool        readData(std::string& data);
    std::string readResponseTerminator();
