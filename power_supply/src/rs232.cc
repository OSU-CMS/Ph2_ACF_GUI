...
void RS232Conn::setReadSuffix(const std::string& suffix) { readSuffix = suffix; }

void RS232Conn::setTerminator(const std::string& term) { terminator = term; }

void RS232Conn::setRemoveEcho(bool removeEcho) { this->removeEcho = removeEcho; }

void RS232Conn::setTimeout(double timeout) { this->timeout = timeout; }

void RS232Conn::setUSBTMCMode(bool mode) { usbtmcmode = mode; }

bool RS232Conn::openPort()
{
    if(port != NULL_FD)
    {
        LOG(logCRITICAL) << "[RS232] Cannot Open Port.\"" << portName << "\"Port already Open ";
        return false;
    }
    int baudr = 0;
    if(!usbtmcmode)
    {
        ...
        default:
            LOG(logCRITICAL) << "[RS232] Invalid baud rate " << baudRate << "!";
            return (1);
            break;
        }
    }
    ...
    // This part has been added by Antonio C.
    struct stat info;
    stat(portName.c_str(), &info);
    struct passwd* pw        = getpwuid(info.st_uid);
    std::string    portOwner = "";
    if(pw != 0) portOwner = pw->pw_name;
    char* buf;
    int   nNameSize = 20;
    buf             = (char*)malloc(nNameSize * sizeof(char));
    int errNo       = getlogin_r(buf, nNameSize);
    if(errNo)
    {
        std::cout << "Problems in getting username: error number" << errNo << ", skipping it" << std::endl;
        std::cout << "BEWARE you need to be root to properly open usb port!" << std::endl;
    }
    else
    {
        if(std::string(buf) != portOwner)
        {
            errNo = system(("sudo chown " + std::string(buf) + " " + portName).c_str());
            if(errNo)
            {
                std::cout << "Failed in changing owner of " << portName.c_str() << " to " << buf << std::endl;
                std::cout << "BEWARE you need to be root to properly open usb port!" << std::endl;
            }
        }
    }
    ...
    if(parity) { newPortSettings.c_cflag |= PARENB | PARODD; }
    else
    {
    ...
    if(flowControl)
    {
        ...
        newPortSettings.c_cc[VSTART] = oldPortSettings.c_cc[VSTART];
        newPortSettings.c_cc[VSTOP] = oldPortSettings.c_cc[VSTOP];
    }
...
void RS232Conn::closePort()
{
    ...
    int status;
    if(ioctl(port, TIOCMGET, &status) == -1 && !usbtmcmode) { LOG(logCRITICAL) << "[RS232] Unable to get portstatus"; }

    status &= ~TIOCM_DTR; /* turn off DTR */
    status &= ~TIOCM_RTS; /* turn off RTS */

    if(ioctl(port, TIOCMSET, &status) == -1 && !usbtmcmode) { LOG(logCRITICAL) << "[RS232] Unable to set portstatus"; }
...
void RS232Conn::writeData(const string& data)
{
    ...
    usleep(1E6 * .02);
    if(removeEcho) { readEcho(data); }
    if(bytesWritten != (dataTerm.size())) { LOG(logWARNING) << "[RS232] Missing Data on Write"; }
}
...
int RS232Conn::readStatus(const string& data)
{
    ...
    if(suffixSize > 0)
    {                                                                                                                  // suffix is ignored if it is unset. i.e. if readSuffix==""
        if(dataSize > suffixSize && data.substr(dataSize - suffixSize, suffixSize) == readSuffix) return MATCH_SUFFIX; // Indicates a match on the readSuffix
    }
...
std::string RS232Conn::readResponseTerminator()
{
    std::string data;
    time_t      tStart;
    time_t      tCurr;
    tStart = time(NULL);
    std::string foundTerminator;
