...
static struct StringToLevelItem stringToLevelMap[] = {{"global", Level::Global},
                                                      {"debug", Level::Debug},
                                                      {"info", Level::Info},
...
Level LevelHelper::convertFromString(const char* levelStr)
{
    ...
    {
        if(base::utils::Str::cStringCaseEq(levelStr, item.levelString)) { return item.level; }
    }
...
ConfigurationType ConfigurationTypeHelper::convertFromString(const char* configStr)
{
    ...
    {
        if(base::utils::Str::cStringCaseEq(configStr, item.configString)) { return item.configType; }
    }
...
Configuration::Configuration(const Configuration& c) : m_level(c.m_level), m_configurationType(c.m_configurationType), m_value(c.m_value) {}
...
Configuration::Configuration(Level level, ConfigurationType configurationType, const std::string& value) : m_level(level), m_configurationType(configurationType), m_value(value) {}
...
Configuration::Predicate::Predicate(Level level, ConfigurationType configurationType) : m_level(level), m_configurationType(configurationType) {}

bool Configuration::Predicate::operator()(const Configuration* conf) const { return ((conf != nullptr) && (conf->level() == m_level) && (conf->configurationType() == m_configurationType)); }

// Configurations

Configurations::Configurations(void) : m_configurationFile(std::string()), m_isFromFile(false) {}

Configurations::Configurations(const std::string& configurationFile, bool useDefaultsForRemaining, Configurations* base) : m_configurationFile(configurationFile), m_isFromFile(false)
{
...
bool Configurations::parseFromFile(const std::string& configurationFile, Configurations* base)
{
    ...
    ELPP_ASSERT((assertionPassed = base::utils::File::pathExists(configurationFile.c_str(), true)) == true, "Configuration file [" << configurationFile << "] does not exist!");
...
bool Configurations::hasConfiguration(ConfigurationType configurationType)
{
    ...
    bool                 result = false;
    LevelHelper::forEachLevel(&lIndex,
                              [&](void) -> bool
                              {
...
bool Configurations::Parser::isLevel(const std::string& line) { return base::utils::Str::startsWith(line, std::string(base::consts::kConfigurationLevel)); }
...
bool Configurations::Parser::isConfig(const std::string& line)
{
    ...
    return line != "" && ((line[0] >= 'A' && line[0] <= 'Z') || (line[0] >= 'a' && line[0] <= 'z')) && (assignment != std::string::npos) && (line.size() > assignment);
}
...
void Logger::configure(const Configurations& configurations)
{
    ...
    if(m_typedConfigurations != nullptr)
    {
        ...
        if(c->hasConfiguration(Level::Global, ConfigurationType::Filename)) { flush(); }
    }
    base::threading::ScopedLock scopedLock(lock());
...
void Logger::flush(Level level, base::type::fstream_t* fs)
{
    ...
    if(fs != nullptr)
    {
        ...
        if(iter != m_unflushedCount.end()) { iter->second = 0; }
...
{
    if((fullPath == "") || (fullPath.find(separator) == std::string::npos)) { return fullPath; }
    std::size_t lastSlashAt = fullPath.find_last_of(separator);
    if(lastSlashAt == 0) { return std::string(separator); }
...
std::string& Str::trim(std::string& str) { return ltrim(rtrim(str)); }

bool Str::startsWith(const std::string& str, const std::string& start) { return (str.length() >= start.length()) && (str.compare(0, start.length(), start) == 0); }

bool Str::endsWith(const std::string& str, const std::string& end) { return (str.length() >= end.length()) && (str.compare(str.length() - end.length(), end.length(), end) == 0); }
...
std::string& Str::replaceAll(std::string& str, const std::string& replaceWhat, const std::string& replaceWith)
{
...
void Str::replaceFirstWithEscape(base::type::string_t& str, const base::type::string_t& replaceWhat, const base::type::string_t& replaceWith)
{
...
#if defined(ELPP_UNICODE)
void Str::replaceFirstWithEscape(base::type::string_t& str, const base::type::string_t& replaceWhat, const std::string& replaceWith)
{
...
std::string DateTime::timevalToString(struct timeval tval, const char* format, const el::base::SubsecondPrecision* ssPrec)
{
    struct ::tm timeInfo;
    buildTimeInfo(&tval, &timeInfo);
    const int kBuffSize        = 30;
    char      buff_[kBuffSize] = "";
    parseFormat(buff_, kBuffSize, format, &timeInfo, static_cast<std::size_t>(tval.tv_usec / ssPrec->m_offset), ssPrec);
...
base::type::string_t DateTime::formatTime(unsigned long long time, base::TimestampUnit timestampUnit)
{
    ...
    {
        if(time <= base::consts::kTimeFormats[i].value) { break; }
        if(base::consts::kTimeFormats[i].value == 1000.0f && time / 1000.0f < 1.9f) { break; }
...
unsigned long long DateTime::getTimeDifference(const struct timeval& endTime, const struct timeval& startTime, base::TimestampUnit timestampUnit)
{
    ...
    // milliseconds
    auto conv = [](const struct timeval& tim) { return static_cast<unsigned long long>((tim.tv_sec * 1000) + (tim.tv_usec / 1000)); };
...
char* DateTime::parseFormat(char* buf, std::size_t bufSz, const char* format, const struct tm* tInfo, std::size_t msec, const base::SubsecondPrecision* ssPrec)
{
    ...
    for(; *format; ++format)
    {
        if(*format == base::consts::kFormatSpecifierChar)
        {
            ...
            case 'g': buf = base::utils::Str::convertAndAddToBuff(msec, ssPrec->m_width, buf, bufLim); continue;
            case 'F': // AM/PM
...
void CommandLineArgs::setArgs(int argc, char** argv)
{
    ...
    for(int i = 1; i < m_argc; ++i)
    {
        ...
        if(v != nullptr && strlen(v) > 0)
        {
            ...
            else { m_paramsWithValue.insert(std::make_pair(key, std::string(v + 1))); }
        }
        if(v == nullptr)
        {
            if(hasParam(m_argv[i])) { ELPP_INTERNAL_INFO(1, "Skipping [" << m_argv[i] << "] arg since it already exists"); }
            else { m_params.push_back(std::string(m_argv[i])); }
        }
    }
}
...
bool CommandLineArgs::hasParam(const char* paramKey) const { return std::find(m_params.begin(), m_params.end(), std::string(paramKey)) != m_params.end(); }

bool CommandLineArgs::empty(void) const { return m_params.empty() && m_paramsWithValue.empty(); }

std::size_t CommandLineArgs::size(void) const { return m_params.size() + m_paramsWithValue.size(); }
...
{
    if(width < 1 || width > 6) { width = base::consts::kDefaultSubsecondPrecision; }
    m_width = width;
    switch(m_width)
    {
...
void LogFormat::updateFormatSpec(void)
{
    ...
    if(hasFlag(base::FormatFlags::User)) { base::utils::Str::replaceFirstWithEscape(m_format, base::consts::kCurrentUserFormatSpecifier, m_currentUser); }
    if(hasFlag(base::FormatFlags::Host)) { base::utils::Str::replaceFirstWithEscape(m_format, base::consts::kCurrentHostFormatSpecifier, m_currentHost); }
...
bool TypedConfigurations::enabled(Level level) { return getConfigByVal<bool>(level, &m_enabledMap, "enabled"); }

bool TypedConfigurations::toFile(Level level) { return getConfigByVal<bool>(level, &m_toFileMap, "toFile"); }

const std::string& TypedConfigurations::filename(Level level) { return getConfigByRef<std::string>(level, &m_filenameMap, "filename"); }

bool TypedConfigurations::toStandardOutput(Level level) { return getConfigByVal<bool>(level, &m_toStandardOutputMap, "toStandardOutput"); }

const base::LogFormat& TypedConfigurations::logFormat(Level level) { return getConfigByRef<base::LogFormat>(level, &m_logFormatMap, "logFormat"); }

const base::SubsecondPrecision& TypedConfigurations::subsecondPrecision(Level level) { return getConfigByRef<base::SubsecondPrecision>(level, &m_subsecondPrecisionMap, "subsecondPrecision"); }

const base::MillisecondsWidth& TypedConfigurations::millisecondsWidth(Level level) { return getConfigByRef<base::MillisecondsWidth>(level, &m_subsecondPrecisionMap, "millisecondsWidth"); }

bool TypedConfigurations::performanceTracking(Level level) { return getConfigByVal<bool>(level, &m_performanceTrackingMap, "performanceTracking"); }

base::type::fstream_t* TypedConfigurations::fileStream(Level level) { return getConfigByRef<base::FileStreamPtr>(level, &m_fileStreamMap, "fileStream").get(); }

std::size_t TypedConfigurations::maxLogFileSize(Level level) { return getConfigByVal<std::size_t>(level, &m_maxLogFileSizeMap, "maxLogFileSize"); }

std::size_t TypedConfigurations::logFlushThreshold(Level level) { return getConfigByVal<std::size_t>(level, &m_logFlushThresholdMap, "logFlushThreshold"); }
...
void TypedConfigurations::build(Configurations* configurations)
{
    ...
    for(Configurations::const_iterator it = configurations->begin(); it != configurations->end(); ++it)
    {
        ...
        else if(conf->configurationType() == ConfigurationType::ToFile) { setValue(conf->level(), getBool(conf->value()), &m_toFileMap); }
        else if(conf->configurationType() == ConfigurationType::ToStandardOutput) { setValue(conf->level(), getBool(conf->value()), &m_toStandardOutputMap); }
        ...
        {
            setValue(Level::Global, base::SubsecondPrecision(static_cast<int>(getULong(conf->value()))), &m_subsecondPrecisionMap);
        }
        else if(conf->configurationType() == ConfigurationType::PerformanceTracking) { setValue(Level::Global, getBool(conf->value()), &m_performanceTrackingMap); }
        else if(conf->configurationType() == ConfigurationType::MaxLogFileSize)
        {
            auto v = getULong(conf->value());
            setValue(conf->level(), static_cast<std::size_t>(v), &m_maxLogFileSizeMap);
            if(v != 0) { withFileSizeLimit.push_back(conf); }
        }
        else if(conf->configurationType() == ConfigurationType::LogFlushThreshold) { setValue(conf->level(), static_cast<std::size_t>(getULong(conf->value())), &m_logFlushThresholdMap); }
    }
...
std::string TypedConfigurations::resolveFilename(const std::string& filename)
{
    ...
    if((dateIndex = resultingFilename.find(dateTimeFormatSpecifierStr.c_str())) != std::string::npos)
    {
        ...
        if(dateIndex != std::string::npos)
        {
            ...
            else { fmt = std::string(base::consts::kDefaultDateTimeFormatInFilename); }
...
Logger* RegisteredLoggers::get(const std::string& id, bool forceCreation)
{
    ...
    if(logger_ == nullptr && forceCreation)
    {
        ...
        for(const std::pair<std::string, base::type::LoggerRegistrationCallbackPtr>& h: m_loggerRegistrationCallbacks)
        {
            ...
            if(callback != nullptr && callback->enabled()) { callback->handle(logger_); }
        }
    }
    return logger_;
}
...
{
    if(id == base::consts::kDefaultLoggerId) { return false; }
...
void VRegistry::setModules(const char* modules)
{
    ...
    if(!ss.str().empty() && level != -1) { insert(ss, static_cast<base::type::VerboseLevel>(level)); }
}
...
bool VRegistry::allowed(base::type::VerboseLevel vlevel, const char* file)
{
    ...
    else
    {
        ...
        {
            if(base::utils::Str::wildCardMatch(baseFilename, it->first.c_str())) { return vlevel <= it->second; }
        }
...
void VRegistry::setFromArgs(const base::utils::CommandLineArgs* commandLineArgs)
{
    ...
    else if(commandLineArgs->hasParamWithValue("--v")) { setLevel(static_cast<base::type::VerboseLevel>(atoi(commandLineArgs->getParamValue("--v")))); }
    else if(commandLineArgs->hasParamWithValue("--V")) { setLevel(static_cast<base::type::VerboseLevel>(atoi(commandLineArgs->getParamValue("--V")))); }
    else if((commandLineArgs->hasParamWithValue("-vmodule")) && vModulesEnabled()) { setModules(commandLineArgs->getParamValue("-vmodule")); }
    else if(commandLineArgs->hasParamWithValue("-VMODULE") && vModulesEnabled()) { setModules(commandLineArgs->getParamValue("-VMODULE")); }
}
...
void Storage::setApplicationArguments(int argc, char** argv)
{
    ...
    if(m_commandLineArgs.hasParamWithValue(base::consts::kLoggingFlagsParam))
    {
        ...
        if(ELPP_DEFAULT_LOGGING_FLAGS == 0x0) { m_flags = userInput; }
        else { base::utils::addFlag<base::type::EnumType>(userInput, &m_flags); }
    }
#endif // defined(ELPP_LOGGING_FLAGS_FROM_ARG)
}
...
void LogDispatchCallback::handle(const LogDispatchData* data)
{
    ...
    if(lock == m_fileLocks.end()) { m_fileLocks.emplace(std::make_pair(filename, std::unique_ptr<base::threading::Mutex>(new base::threading::Mutex))); }
}
#else
void LogDispatchCallback::handle(const LogDispatchData* /*data*/) {}
#endif
...
base::type::string_t DefaultLogBuilder::build(const LogMessage* logMessage, bool appendNewLine) const
{
    ...
    if(logFormat->hasFlag(base::FormatFlags::DateTime))
    {
        ...
        base::utils::Str::replaceFirstWithEscape(
            logLine, base::consts::kDateTimeFormatSpecifier, base::utils::DateTime::getDateTime(logFormat->dateTimeFormat().c_str(), &tc->subsecondPrecision(logMessage->level())));
    }
...
{
    m_logger                = logger;
    m_containerLogSeparator = ELPP->hasFlag(LoggingFlag::NewLineForContainer) ? ELPP_LITERAL("\n    ") : ELPP_LITERAL(", ");
}
...
Writer& Writer::construct(int count, const char* loggerIds, ...)
{
    ...
    else { initializeLogger(std::string(loggerIds)); }
    m_messageBuilder.initialize(m_logger);
    return *this;
}
...
void Writer::initializeLogger(const std::string& loggerId, bool lookup, bool needLock)
{
    ...
    else
    {
        ...
        else { m_proceed = m_logger->enabled(m_level); }
    }
}
...
void Writer::triggerDispatch(void)
{
    try
    {
        if(m_proceed)
        {
            if(m_msg == nullptr)
            {
                LogMessage msg(m_level, m_file, m_line, m_func, m_verboseLevel, m_logger);
                base::LogDispatcher(m_proceed, &msg, m_dispatchAction).dispatch();
            }
            else { base::LogDispatcher(m_proceed, m_msg, m_dispatchAction).dispatch(); }
        }
        if(m_logger != nullptr)
        {
            m_logger->stream().str(ELPP_LITERAL(""));
...
PerformanceTracker::~PerformanceTracker(void)
{
    ...
    if(m_enabled)
    {
        ...
        if(m_scopedLog)
        {
            ...
            {
                callback = h.second.get();
                if(callback != nullptr && callback->enabled()) { callback->handle(&data); }
            }
        }
    }
...
{
    if(sig == SIGINT && ELPP->hasFlag(el::LoggingFlag::IgnoreSigInt)) { return; }
    std::stringstream ss;
    ss << "CRASH HANDLED; ";
    ss << crashReason(sig);
#if ELPP_STACKTRACE
...
static inline void crashAbort(int sig) { base::utils::abort(sig, std::string()); }
...
void Helpers::logCrashReason(int sig, bool stackTraceIfAvailable, Level level, const char* logger) { el::base::debug::logCrashReason(sig, stackTraceIfAvailable, level, logger); }
...
Logger* Loggers::reconfigureLogger(const std::string& identity, const Configurations& configurations) { return Loggers::reconfigureLogger(Loggers::getLogger(identity), configurations); }

Logger* Loggers::reconfigureLogger(const std::string& identity, ConfigurationType configurationType, const std::string& value)
{
...
const Configurations* Loggers::defaultConfigurations(void) { return ELPP->registeredLoggers()->defaultConfigurations(); }

const base::LogStreamsReferenceMapPtr Loggers::logStreamsReference(void) { return ELPP->registeredLoggers()->logStreamsReference(); }
...
void Loggers::setVerboseLevel(base::type::VerboseLevel level) { ELPP->vRegistry()->setLevel(level); }

base::type::VerboseLevel Loggers::verboseLevel(void) { return ELPP->vRegistry()->level(); }
...
const std::string VersionInfo::releaseDate(void) { return std::string("24-11-2018 0728hrs"); }
