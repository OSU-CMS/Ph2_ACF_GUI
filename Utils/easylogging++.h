...
{
  public:
    std::size_t operator()(const el::Level& l) const { return hash<el::base::type::EnumType>{}(static_cast<el::base::type::EnumType>(l)); }
};
} // namespace std
namespace el
{
...
const struct
{
    ...
    {{1000.0f, ELPP_LITERAL("us")}, {1000.0f, ELPP_LITERAL("ms")}, {60.0f, ELPP_LITERAL("seconds")}, {60.0f, ELPP_LITERAL("minutes")}, {24.0f, ELPP_LITERAL("hours")}, {7.0f, ELPP_LITERAL("days")}};
...
class SubsecondPrecision
{
  public:
    ...
    bool         operator==(const SubsecondPrecision& ssPrec) { return m_width == ssPrec.m_width && m_offset == ssPrec.m_offset; }
    int          m_width;
...
template <typename T>
static typename std::enable_if<std::is_pointer<T*>::value, void>::type safeDelete(T*& pointer)
{
...
class HitCounter
{
  public:
    ...
    HitCounter(const char* filename, base::type::LineNumber lineNumber) : m_filename(filename), m_lineNumber(lineNumber), m_hitCounts(0) {}
    ...
    {
      public:
        Predicate(const char* filename, base::type::LineNumber lineNumber) : m_filename(filename), m_lineNumber(lineNumber) {}
...
class RegisteredLoggers : public base::utils::Registry<Logger, std::string>
{
  public:
    ...
    inline LogStreamsReferenceMapPtr logStreamsReference(void) { return m_logStreamsReference; }
    ...
    , public base::threading::ThreadSafe
{
  public:
    ...
    inline bool validateEveryNCounter(const char* filename, base::type::LineNumber lineNumber, std::size_t occasion) { return hitCounters()->validateEveryN(filename, lineNumber, occasion); }

    inline bool validateAfterNCounter(const char* filename, base::type::LineNumber lineNumber, std::size_t n) { return hitCounters()->validateAfterN(filename, lineNumber, n); }

    inline bool validateNTimesCounter(const char* filename, base::type::LineNumber lineNumber, std::size_t n) { return hitCounters()->validateNTimes(filename, lineNumber, n); }
    ...
    base::threading::Mutex& customFormatSpecifiersLock() { return m_customFormatSpecifiersLock; }
    ...
    inline std::string getThreadName(const std::string& threadId)
    {
        ...
        if(it == m_threadNames.end()) { return threadId; }
        return it->second;
    }
...
class Writer : base::NoCopy
{
  public:
    Writer(Level                    level,
        ...
        : m_msg(nullptr), m_level(level), m_file(file), m_line(line), m_func(func), m_verboseLevel(verboseLevel), m_logger(nullptr), m_proceed(false), m_dispatchAction(dispatchAction)
    {
    }

    Writer(LogMessage* msg, base::DispatchAction dispatchAction = base::DispatchAction::NormalLog)
        : m_msg(msg), m_level(msg != nullptr ? msg->level() : Level::Unknown), m_line(0), m_logger(nullptr), m_proceed(false), m_dispatchAction(dispatchAction)
    {
    }
...
#define ELPP_WRITE_LOG_EVERY_N(writer, occasion, level, dispatchAction, ...)                                                                                                                           \
    ELPP->validateEveryNCounter(__FILE__, __LINE__, occasion) && writer(level, __FILE__, __LINE__, ELPP_FUNC, dispatchAction).construct(el_getVALength(__VA_ARGS__), __VA_ARGS__)
#define ELPP_WRITE_LOG_AFTER_N(writer, n, level, dispatchAction, ...)                                                                                                                                  \
    ELPP->validateAfterNCounter(__FILE__, __LINE__, n) && writer(level, __FILE__, __LINE__, ELPP_FUNC, dispatchAction).construct(el_getVALength(__VA_ARGS__), __VA_ARGS__)
#define ELPP_WRITE_LOG_N_TIMES(writer, n, level, dispatchAction, ...)                                                                                                                                  \
    ELPP->validateNTimesCounter(__FILE__, __LINE__, n) && writer(level, __FILE__, __LINE__, ELPP_FUNC, dispatchAction).construct(el_getVALength(__VA_ARGS__), __VA_ARGS__)
#if defined(ELPP_FEATURE_ALL) || defined(ELPP_FEATURE_PERFORMANCE_TRACKING)
class PerformanceTrackingData
{
  public:
    ...
    explicit PerformanceTrackingData(DataType dataType) : m_performanceTracker(nullptr), m_dataType(dataType), m_firstCheckpoint(false), m_file(""), m_line(0), m_func("") {}
    ...
    , public Loggable
{
  public:
    ...
    void         checkpoint(const std::string& id = std::string(), const char* file = __FILE__, base::type::LineNumber line = __LINE__, const char* func = "");
    inline Level level(void) const { return m_level; }
...
class Helpers : base::StaticClass
{
  public:
    ...
    /// @brief Sets thread name for current thread. Requires std::thread
    static inline void        setThreadName(const std::string& name) { ELPP->setThreadName(name); }
    static inline std::string getThreadName() { return ELPP->getThreadName(base::threading::getCurrentThreadId()); }
#if defined(ELPP_FEATURE_ALL) || defined(ELPP_FEATURE_CRASH_LOG)
    ...
    static inline void reserveCustomFormatSpecifiers(std::size_t size) { ELPP->m_customFormatSpecifiers.reserve(size); }
...
#if ELPP_ASYNC_LOGGING
#define INITIALIZE_EASYLOGGINGPP ELPP_INIT_EASYLOGGINGPP(new el::base::Storage(el::LogBuilderPtr(new el::base::DefaultLogBuilder()), new el::base::AsyncDispatchWorker()))
#else
