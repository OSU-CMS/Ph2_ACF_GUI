...
ArgvParser::ArgvParser() : max_key(1), help_option(0) // must be smaller than max_key initially
...
bool ArgvParser::isDefinedOption(const string& _name) const { return (option2key.find(_name) != option2key.end()); }
...
ArgvParser::ParserResults ArgvParser::parse(int _argc, char** _argv)
{
        ...
        else // can be a long or multiple short options at this point
        {
            ...
            else // handle short options
            {
                ...
                if(splitOptionAndValue(argument, option, value))
                {
                    ...
                    {
                        error_option = option;                       // store the option that caused the error
                        return (ParserMalformedMultipleShortOption); // return error code if option has more than one
                                                                     // character
                    }
    ...
    for(it = option2attribute.begin(); it != option2attribute.end(); it++)
    {
        ...
        if(it->second & OptionRequired)
        {
            ...
            if(option2value.find(it->first) == option2value.end())
            {
                ...
                for(list<string>::const_iterator alt = alternatives.begin(); alt != alternatives.end(); ++alt)
                {
                    ++count;
...
unsigned int ArgvParser::arguments() const { return (argument_container.size()); }
...
const vector<string>& ArgvParser::allArguments() const { return (argument_container); }
...
string ArgvParser::usageDescription(unsigned int _width) const
{
    ...
    for(Key2AttributeMap::const_iterator it = option2attribute.begin(); it != option2attribute.end(); ++it)
    {
    ...
    for(std::map<int, std::string>::const_iterator alt = errorcode2descr.begin(); alt != errorcode2descr.end(); ++alt)
    {
        ostringstream code;
        code << alt->first;
...
const string& ArgvParser::errorOption() const { return (error_option); }
...
bool ArgvParser::defineOption(const string& _name, const string& _descr, OptionAttributes _attrs)
{
...
bool ArgvParser::defineOptionAlternative(const string& _original, const string& _alternative)
{
...
bool ArgvParser::setHelpOption(const string& _shortname, const string& _longname, const string& _descr)
{
...
void ArgvParser::addErrorCode(int _code, const string& _descr) { errorcode2descr[_code] = _descr; }

void ArgvParser::setIntroductoryDescription(const string& _descr) { intro_description = _descr; }
...
bool CommandLineProcessing::splitOptionAndValue(const string& _string, string& _option, string& _value)
{
...
bool CommandLineProcessing::expandRangeStringToUInt(const std::string& _string, std::vector<unsigned int>& _expanded)
{
    list<string> tokens;
    // split string by delimiter
    splitString(tokens, _string, ",");
